...
contract Create {
    ...
    require(voters[_address].voter_allowed == 1, "Voter does not exist");
    ...
    // Removing the voter's address from the votersAddress array
    for (uint256 i = 0; i < votersAddress.length; i++) {
        if (votersAddress[i] == _address) {
            // Replace the removed address with the last address in the array
            votersAddress[i] = votersAddress[votersAddress.length - 1];
            // Remove the last element from the array
            votersAddress.pop();
            break;
        }
